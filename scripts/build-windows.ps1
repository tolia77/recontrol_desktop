...
# One-command Windows build: fetch FFmpeg -> publish win-x64 --self-contained -> ISCC
#
...
# Step 1: Fetch FFmpeg DLLs (if not already present)
...
if (-not (Test-Path $avcodecDll)) {
    Write-Host "`n[1/4] FFmpeg DLLs not found -- invoking fetch-ffmpeg-win.ps1"
    & (Join-Path $PSScriptRoot "fetch-ffmpeg-win.ps1")
} else {
    Write-Host "`n[1/4] FFmpeg DLLs already present (avcodec-61.dll found) -- skipping fetch"
}

# Step 2: dotnet publish (self-contained, win-x64, Release)
$publishDir = Join-Path $RepoRoot "publish-win"
$csprojPath = Join-Path $RepoRoot "ReControl.Desktop\ReControl.Desktop.csproj"

Write-Host "`n[2/4] Publishing self-contained win-x64 release to: $publishDir"
dotnet publish $csprojPath -c Release -r win-x64 --self-contained -o $publishDir
if ($LASTEXITCODE -ne 0) { throw "dotnet publish failed with exit code $LASTEXITCODE" }

# Step 3: Verify publish output is complete
Write-Host "`n[3/4] Verifying publish output..."
...
if (-not $allGood) {
    throw "Publish output is incomplete -- see MISSING entries above. Ensure the csproj has ffmpeg ItemGroup and appsettings.json None Update."
}

# Step 4: Run ISCC to produce the installer
Write-Host "`n[4/4] Running Inno Setup compiler (ISCC)..."
